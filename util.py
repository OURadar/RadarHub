...
def quick_check(data, file):
    unixTime = data["time"]
    timeString = time.strftime(r"%Y%m%d-%H%M%S", time.gmtime(unixTime))
...
def load_by_url(server="https://radarhub.arrc.ou.edu", pathway="px1000", month="202408"):
    ...
    hour = random.choice(indices)
    print(f"Selected hour: {hour}")

    url = f"{server}/data/table/{pathway}/{day}-{hour:02d}00/"
    content = get_url(url)
...
def upload(source="/data/moment", **kwargs):
    ...
    execute = not kwargs.get("noexec", False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", dest="noexec", action="store_true", help="do not execute, just show the command")
    ...
    if args.verb[0] == "test":
        ...
        for folder in args.source:
            ...
            upload(source=folder, noexec=args.noexec, verbose=args.verbose)
