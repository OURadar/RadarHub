...
function reviseOpacityV1(buffer, geometry) {
  ...
  for (let k = 0; k < buffer.count; k++) {
    ...
    if (z > 0.98 || x > 0.95 || x < -0.95 || y > 0.95 || y < -0.95) {
      ...
      visibility.push(1);
      const p = [x * geometry.viewport.width, y * geometry.viewport.height];
...
function reviseOpacityV2(buffer, geometry, verbose = 1) {
  ...
  if (verbose) console.log(survived);
...
function revise(buffer) {
  ...
  t1 = new Date().getTime();
  console.log(`getGeometry: ${(t1 - t2) / count} s`);
  ...
  t0 = new Date().getTime();
  console.log(`reviseOpacityV2: ${(t0 - t1) / count} s`);
  ...
  t0 = new Date().getTime();
  console.log(`reviseOpacityV1: ${(t0 - t1) / count} s`);
}
