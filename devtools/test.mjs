...
async function loadUsingShapefileJS(filePath) {
  try {
    const data = readFileSync(filePath);
    const shape = await Shapefile.load(data);
    console.log(shape);
    // console.log(shape.intrstat.contents);
  } catch (error) {
...
async function convert({ src, keys, isLabel }) {
  const dst = src.split(".").slice(0, -1).join(".").concat(".stq2.json");
  ...
  try {
    ...
    if (!isLabel) {
      ...
      console.log(`nameKey = ${nameKey}  weightKey = ${weightKey}  populationKey = ${populationKey}`);
...
// file = "../frontend/static/maps/United States/intrstat.shp";
